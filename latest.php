    ...
    **/
    ...
    if ($hosts) {
        ...
        if (!$singleHostSelected) {
            $sortFields = ($sortField === 'host') ? array(
                array(
                    'field' => 'name',
                    'order' => $sortOrder
                )
    ...
    $filterTable->addRow(array(
        ...
        new CCol(array(
                ...
                new CButton('application_name', _('Select'), 'return PopUp("popup.php?srctbl=applications&srcfld1=name&real_hosts=1&dstfld1=application' . '&with_applications=1&dstfrm=' . $filterForm->getName() . '");', 'filter-select-button')
            ), 'inputcol'),
    ...
    foreach($items as $key => $item) {
        ...
        if ($lastHistory && $prevHistory && ($item['value_type'] == ITEM_VALUE_TYPE_FLOAT || $item['value_type'] == ITEM_VALUE_TYPE_UINT64) && (bcsub($lastHistory['value'], $prevHistory['value'], $digits) != 0)) {
        ...
        if ($filter['showDetails']) {
            ...
            $row = new CRow(array(
                ...
                new CCol(new CSpan(($item['type'] == ITEM_TYPE_SNMPTRAP || $item['type'] == ITEM_TYPE_TRAPPER) ? UNKNOWN_VALUE : $item['delay'], $stateCss)),

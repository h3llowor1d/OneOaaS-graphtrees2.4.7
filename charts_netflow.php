<?php
    ...
    **/
    ...
    $page['title'] = _('Netflow total');
    $page['file'] = 'charts_netflow.php';
    ...
    if (hasRequest('filterState')) {
        CProfile::update('web.charts_netflow.filter.state', getRequest('filterState'), PROFILE_TYPE_INT);
    }

    if (isset($_REQUEST['favobj'])) {
        if (getRequest('favobj') === 'timelinefixedperiod' && hasRequest('favid')) {
            CProfile::update('web.charts_netflow.timelinefixed', getRequest('favid'), PROFILE_TYPE_INT);
        }
    ...
    // render view
    $chartsView = new CView('monitoring.charts_netflow', $data);
    $chartsView->render();
    $chartsView->show();

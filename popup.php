<?php
    ...
    **/
    ...
    $fields = array(
        ...
        'from'                    => array(T_ZBX_STR, O_OPT, null, null, null),
        ...
        'item_key'                => array(T_ZBX_STR, O_OPT, null, null, null),
    ...
    if (getRequest('only_hostid')) {
        ...
        if (getRequest('hostid') && !API::Host()->isReadable(array($_REQUEST['hostid'])) || getRequest('groupid') && !API::HostGroup()->isReadable(array($_REQUEST['groupid']))) {
            access_deny();
        }
    }
 ...
 */ elseif ($srctbl == 'templates') {
        ...
        foreach($templates as &$template) {
            ...
            $jsAction = 'javascript: addValue(' . zbx_jsvalue($reference) . ', ' . zbx_jsvalue($template['templateid']) . ', ' . $parentId . ');';
        ...
        if ($multiselect) {
            $button = new CButton('select', _('Select'), "javascript: addSelectedValues('templates', " . zbx_jsvalue($reference) . ', ' . $parentId . ');');
            $table->setFooter(new CCol($button, 'right'));
        }
 ...
 */ elseif ($srctbl == 'hosts') {
        ...
        $table->setHeader(array(
            ($multiselect ? new CCheckBox('all_hosts', null, "javascript: checkAll('" . $form->getName() . "', 'all_hosts', 'hosts');") : null),
            _('Name')
        ));
        ...
        foreach($hosts as &$host) {
            ...
            $jsAction = 'javascript: addValue(' . zbx_jsvalue($reference) . ', ' . zbx_jsvalue($host['hostid']) . ', ' . $parentId . ');';
        ...
        if ($multiselect) {
            $button = new CButton('select', _('Select'), "javascript: addSelectedValues('hosts', " . zbx_jsvalue($reference) . ', ' . $parentId . ');');
            $table->setFooter(new CCol($button, 'right'));
        }
 ...
 */ elseif ($srctbl == 'host_groups') {
        ...
        $table->setHeader(array(
            ($multiselect ? new CCheckBox('all_hostgroups', null, "javascript: checkAll('" . $form->getName() . "', 'all_hostgroups', 'hostGroups');") : null),
            _('Name')
        ));
        ...
        insert_js('var popupReference = ' . zbx_jsvalue($data, true) . ';');
        //echo 'var popupReference = ' . zbx_jsvalue($data, true) . ';';
 ...
 */ elseif ($srctbl == 'triggers') {
        ...
        foreach($triggers as $trigger) {
            ...
            $table->addRow(array(
                ...
                new CSpan(triggerIndicator($trigger['status'], $trigger['state']), triggerIndicatorStyle($trigger['status'], $trigger['state']))
            ));
 ...
 */ elseif ($srctbl === 'items' || $srctbl === 'item_prototypes') {
        ...
        foreach($items as $item) {
            ...
            $parentId = $dstfld1 ? zbx_jsvalue($dstfld1) : 'null';
            if ($multiselect) {
                $js_action = 'javascript: addValue(' . zbx_jsvalue($reference) . ', ' . zbx_jsvalue($item['itemid']) . ',' . $parentId . ');';
            } else {
                $values = array();
                for($i = 1; $i <= $dstfldCount; $i++) {
 ...
 */ elseif ($srctbl == 'applications') {
        ...
        $table->setHeader(array(
            ($multiselect ? new CCheckBox('all_applications', null, "javascript: checkAll('" . $form->getName() . "', 'all_applications', 'applications');") : null),
            _('Name')
        ));
 ...
 */ elseif ($srctbl === 'graphs' || $srctbl === 'graph_prototypes') {
        ...
        if ($multiselect) {
            $header = array(
                array(
                    new CCheckBox('all_graphs', null, "javascript: checkAll('" . $form->getName() . "', 'all_graphs', 'graphs');"),
                    _('Description')
                ),
                _('Graph type')
            );
 ...
 */ elseif ($srctbl == 'sysmaps') {
        ...
        if ($multiselect) {
            $header = array(
                array(
                    new CCheckBox('all_sysmaps', null, "javascript: checkAll('" . $form->getName() . "', 'all_sysmaps', 'sysmaps');"),
                    _('Name')
                )
            );
        } else {
        ...
        foreach($sysmaps as $sysmap) {
            ...
            if ($multiselect) {
                $description = new CCol(array(
                    new CCheckBox('sysmaps[' . zbx_jsValue($sysmap[$srcfld1]) . ']', null, null, $sysmap['sysmapid']),
                    $description
                ));
            }
 ...
 */ elseif ($srctbl == 'slides') {
        ...
        if ($multiselect) {
            $header = array(
                array(
                    new CCheckBox('all_slides', null, "javascript: checkAll('" . $form->getName() . "', 'all_slides', 'slides');"),
                    _('Name')
                ),
            );
        } else {
        ...
        foreach($dbSlideshows as $dbSlideshow) {
            ...
            if ($multiselect) {
                $name = new CCol(array(
                    new CCheckBox('slides[' . zbx_jsValue($dbSlideshow[$srcfld1]) . ']', null, null, $dbSlideshow['slideshowid']),
                    $name
                ));
            }
            $table->addRow($name);
 ...
 */ elseif ($srctbl == 'screens') {
        ...
        if ($multiselect) {
            $header = array(
                array(
                    new CCheckBox('all_screens', null, "javascript: checkAll('" . $form->getName() . "', 'all_screens', 'screens');"),
                    _('Name')
                ),
            );
        } else {
        ...
        foreach($screens as $screen) {
            ...
            if ($multiselect) {
                $name = new CCol(array(
                    new CCheckBox('screens[' . zbx_jsValue($screen[$srcfld1]) . ']', null, null, $screen['screenid']),
                    $name
                ));
            }
            $table->addRow($name);
 ...
 */ elseif ($srctbl == 'scripts') {
        ...
        if ($multiselect) {
            $header = array(
                array(
                    new CCheckBox('all_scripts', null, "javascript: checkAll('" . $form->getName() . "', 'all_scripts', 'scripts');"),
                    _('Name')
                ),
                _('Execute on'),
                _('Commands')
        ...
        foreach($scripts as $script) {
            ...
            if ($multiselect) {
                $description = new CCol(array(
                    new CCheckBox('scripts[' . zbx_jsValue($script[$srcfld1]) . ']', null, null, $script['scriptid']),
                    $description
                ));
            }
        ...
        $form->addItem($table);
        $form->show();
    } elseif ($srctbl == 'item_key') {
        $form = new CForm();
        $form->setName('item_key_form');
        $form->setAttribute('id', 'item_key');

        $table = new CTableInfo(_('No item keys found.'));
        $table->setHeader(array(
            ($multiselect ? new CCheckBox('all_itemkeys', null, "javascript: checkAll('" . $form->getName() . "', 'all_itemkeys', 'item_key');") : null),
            _('Name')
        ));
        ...
        if ($groupids) {
            ...
            WHERE 1 = 1
              ...
              ORDER BY i.`key_`
        ";
            $itemkeys = DBfetchArray(DBselect($sql));
        }else{
            error("Host groups must be selected");
        }
        $data = array();
        $parentId = $dstfld1 ? zbx_jsvalue($dstfld1) : 'null';
        ...
        if(!empty($itemkeys)){
            foreach($itemkeys as &$itemkey) {
                $name = new CSpan($itemkey['key_'], 'link');
                $name->attr('id', 'spanid' . $itemkey['key_']);

                $jsAction = 'javascript: addValue(' . zbx_jsvalue($reference) . ', ' . zbx_jsvalue($itemkey['key_']) . ', ' . $parentId . ');';
                if ($multiselect) {
                    $checkBox = new CCheckBox('item_key[' . $itemkey['key_'] . ']', null, null, $itemkey['key_']);
                }

                // check for existing
                $name->setAttribute('onclick', $jsAction . ' jQuery(this).removeAttr("onclick");');

                $data[$itemkey['key_']] = array(
                    'id'   => $itemkey['itemid'],
                    'name' => $itemkey['key_']
                );
        ...
        if ($multiselect) {
            $button = new CButton('select', _('Select'), "javascript: addSelectedValues('item_key', " . zbx_jsvalue($reference) . ', ' . $parentId . ');');
            $table->setFooter(new CCol($button, 'right'));
        }

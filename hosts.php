    ...
    **/
    ...
    if (isset($_REQUEST['unlink']) || isset($_REQUEST['unlink_and_clear'])) {
        ...
        try {
            ...
            if (isset($visible['inventory_mode'])) {
                ...
                $newValues['inventory'] = ($newValues['inventory_mode'] == HOST_INVENTORY_DISABLED) ? array() : getRequest('host_inventory', array());
            }
        ...
        try {
            ...
            if (!$create && $dbHost['flags'] == ZBX_FLAG_DISCOVERY_CREATED) {
                $host = array(
                    ...
                    'inventory'   => (getRequest('inventory_mode') == HOST_INVENTORY_DISABLED) ? array() : getRequest('host_inventory', array())
                );
            } else {
                // templates
                $templates = array();
    ...
    if (hasRequest('action') && getRequest('action') == 'host.massupdateform' && hasRequest('hosts')) {
        ...
        $filterTable->addRow(array(
            array(
                array(bold(_('Name')), SPACE . _('like') . NAME_DELIMITER),
                new CTextBox('filter_host', $filter['host'], 20)
            ),
            array(
                array(bold(_('DNS')), SPACE . _('like') . NAME_DELIMITER),
                new CTextBox('filter_dns', $filter['dns'], 20)
            ),
            array(
                array(bold(_('IP')), SPACE . _('like') . NAME_DELIMITER),
                new CTextBox('filter_ip', $filter['ip'], 20)
            ),
        ...
        $paging = getPagingLine($hosts, $sortOrder);
        ...
        foreach($hosts as $host) {
            ...
            $applications = array(
                new CLink(_('Applications'), 'applications.php?groupid=' . $_REQUEST['groupid'] . '&hostid=' . $host['hostid']),
                ' (' . $host['applications'] . ')'
            );
            $items = array(
            ...
            $triggers = array(
                new CLink(_('Triggers'), 'triggers.php?groupid=' . $_REQUEST['groupid'] . '&hostid=' . $host['hostid']),
                ' (' . $host['triggers'] . ')'
            );
            $graphs = array(
                new CLink(_('Graphs'), 'graphs.php?groupid=' . $_REQUEST['groupid'] . '&hostid=' . $host['hostid']),
                ' (' . $host['graphs'] . ')'
            );
            $discoveries = array(
            ...
            if (empty($host['parentTemplates'])) {
                ...
                foreach($host['parentTemplates'] as $template) {
                    ...
                    $caption = array(
                        new CLink(CHtml::encode($template['name']), 'templates.php?form=update&templateid=' . $template['templateid'], 'unknown')
                    );

<?php
    ...
    **/
    ...
    function get_host_by_itemid($itemids) {
        ...
        $db_hostsItems = DBselect('SELECT i.itemid,h.*' . ' FROM hosts h,items i' . ' WHERE i.hostid=h.hostid' . ' AND ' . dbConditionInt('i.itemid', $itemids));
    ...
    function getUnlinkableHostIds(array $groupIds, array $hostIds) {
        ...
        $dbResult = DBselect('SELECT hg.hostid' . ' FROM hosts_groups hg' . ' WHERE ' . dbConditionInt('hg.groupid', $groupIds, true) . ' AND ' . dbConditionInt('hg.hostid', $hostIds) . ' GROUP BY hg.hostid');
    ...
    function getDeletableHostGroupIds(array $groupIds) {
        ...
        $dbResult = DBselect('SELECT hg.hostid' . ' FROM hosts_groups hg' . ' WHERE ' . dbConditionInt('hg.groupid', $groupIds));
        ...
        $dbResult = DBselect('SELECT g.groupid' . ' FROM groups g' . ' WHERE g.internal=' . ZBX_NOT_INTERNAL_GROUP . ' AND ' . dbConditionInt('g.groupid', $groupIds) . ' AND NOT EXISTS (' . 'SELECT NULL' . ' FROM hosts_groups hg' . ' WHERE g.groupid=hg.groupid' . ($hostIds ? ' AND ' . dbConditionInt('hg.hostid', $hostIds, true) : '') . ')');

<?php
    ...
    **/
    ...
    $gcb = $this->data['pageFilter']->getGraphsCB();
    $cbItems = $gcb->items;
    foreach($cbItems as $k=>$item){
        if(!preg_match('/Network traffic|not selected/',$item)){
            unset($cbItems[$k]);
        }
    }
    $gcb->items = $cbItems ;
    $chartForm->addItem(array(SPACE . _('Graph') . SPACE, $gcb));

    $chartsWidget->addFlicker(new CDiv(null, null, 'scrollbar_cntr'), CProfile::get('web.charts_netflow.filter.state', 1));

    if ($this->data['graphid']) {
        $chartsWidget->addPageHeader(_('GRAPHS'), array(
            get_icon('favourite', array('fav'    => 'web.favorite.graphids',
                                        'elname' => 'graphid',

    ...
    **/
    ...
    if ($page['type'] == PAGE_TYPE_HTML) {
        if (!defined('ZBX_PAGE_NO_MENU') && !defined('ZBX_PAGE_NO_FOOTER')) {
            ...
            $conString = (CWebUser::$data['userid'] == 0) ? _('Not connected') : _s('Connected as \'%1$s\'', CWebUser::$data['alias']);

            $table->addRow(array(
                new CCol(new CLink(_s('Zabbix %1$s Copyright %2$s-%3$s by Zabbix SIA', ZABBIX_VERSION, ZABBIX_COPYRIGHT_FROM, ZABBIX_COPYRIGHT_TO), ZABBIX_HOMEPAGE, 'highlight', null, true), 'center'),
                new CCol(array(

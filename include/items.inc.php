<?php
    ...
    **/
    ...
    function update_item_status($itemids, $status) {
        ...
        while ($item = DBfetch($db_items)) {
            ...
            if ($status != $old_status) {
                $result &= DBexecute('UPDATE items SET status=' . zbx_dbstr($status) . ' WHERE itemid=' . zbx_dbstr($item['itemid']));
                if ($result) {
    ...
    function get_item_by_key($key, $host = '') {
        ...
        $sql = 'SELECT DISTINCT i.*' . ' FROM items i ' . $sql_from . ' WHERE i.key_=' . zbx_dbstr($key) . $sql_where;
    ...
    function get_item_by_itemid_limited($itemid) {
        $row = DBfetch(DBselect('SELECT i.itemid,i.interfaceid,i.name,i.key_,i.hostid,i.delay,i.history,i.status,i.type,i.lifetime,' . 'i.snmp_community,i.snmp_oid,i.value_type,i.data_type,i.trapper_hosts,i.port,i.units,i.multiplier,' . 'i.delta,i.snmpv3_contextname,i.snmpv3_securityname,i.snmpv3_securitylevel,i.snmpv3_authprotocol,' . 'i.snmpv3_authpassphrase,i.snmpv3_privprotocol,i.snmpv3_privpassphrase,i.formula,i.trends,i.logtimefmt,' . 'i.valuemapid,i.delay_flex,i.params,i.ipmi_sensor,i.templateid,i.authtype,i.username,i.password,' . 'i.publickey,i.privatekey,i.flags,i.description,i.inventory_link' . ' FROM items i' . ' WHERE i.itemid=' . zbx_dbstr($itemid)));
        if ($row) {
            return $row;
        }
        error(_s('No item with itemid "%1$s".', $itemid));
    ...
    function get_same_item_for_host($item, $dest_hostids) {
        ...
        if (isset($itemid)) {
            $db_items = DBselect('SELECT src.*' . ' FROM items src,items dest' . ' WHERE dest.itemid=' . zbx_dbstr($itemid) . ' AND src.key_=dest.key_' . ' AND ' . dbConditionInt('src.hostid', $dest_hostids));
    ...
    function get_same_applications_for_host(array $applicationIds, $hostId) {
        ...
        $dbApplications = DBselect('SELECT a1.applicationid AS dstappid,a2.applicationid AS srcappid' . ' FROM applications a1,applications a2' . ' WHERE a1.name=a2.name' . ' AND a1.hostid=' . zbx_dbstr($hostId) . ' AND ' . dbConditionInt('a2.applicationid', $applicationIds));
    ...
    function getItemFunctionalValue($item, $function, $parameter) {
        ...
        if (!isset($historyTables[$item['value_type']])) {
            ...
            $result = DBselect('SELECT ' . $function . '(value) AS value' . ' FROM ' . $historyTables[$item['value_type']] . ' WHERE clock>' . (time() - $parameter) . ' AND itemid=' . zbx_dbstr($item['itemid']) . ' HAVING COUNT(*)>0' // necessary because DBselect() return 0 if empty data set, for graph templates
            );
    ...
    function isKeyIdChar($char) {
        return (($char >= 'a' && $char <= 'z') || $char == '.' || $char == '_' || $char == '-' || ($char >= 'A' && $char <= 'Z') || ($char >= '0' && $char <= '9'));
    }
    ...
    function getGraphsByGroupid($id = null){
        $result = array();
        $groupid = $id;
    ...
    function getGraphsByHostid($id = null){
        $result = array();
        $hostid = $id;
    ...
    function getGraphsByApplicationid($id = null){
        $result = array();
        $applicationid = $id;
    ...
    function getItemsByApplicationid($id = null){
        $result = array();
        $applicationid = $id;
     ...
     * item_key get items
     */
    function getItemsByItemid($ids = array()){
        $result = array();
        if(!is_array($ids)){
            zbx_value2array($ids);
        }
            ...
            if($id){
                $sql = "
                    ...
                    FROM
                      ...
                      LEFT JOIN items i2
                        ON i2.key_ = i1.key_
                    WHERE i2.itemid = ".zbx_dbstr($id)."
                      AND i1.`interfaceid` IS NOT NULL
                      AND i1.`templateid` IS NOT NULL
                ";
            }else{
                continue;
            }

            $temp = DBfetchArray(DBselect($sql));
            if($temp){
                $result[$temp[0]['key_']] = $temp;
            }
        }
    ...
    function getDataFromHistory($options = array()){
        $result = array();
        ...
        $t1 = time() - 86400;  // a day ago from now
        $etime = time();
        if(!empty($stime) && !empty($period)){
        ...
        $sql = "
            ...
            WHERE i.hostid = ".zbx_dbstr($hostid)."
              AND i.key_ = ".zbx_dbstr($itemkey)."
              AND i.interfaceid IS NOT NULL
        ";
        $item = DBfetchArray(DBselect($sql));
        ...
        if($item){
            if($flags == 2){
                $sql = "
                    ...
                    FROM
                      item_discovery id,
                      items i
                    WHERE parent_itemid = {$item[0]['itemid']}
                      AND i.`itemid` = id.`itemid`
                ";

                $discoveryItems = DBfetchArray(DBselect($sql));
            }else{
                $discoveryItems = $item;
            }
            ...
            foreach($discoveryItems as $item){
                ...
                $fileds = "
                    ...
                    AVG(h.value) AS 'avg'
                ";
                if(strpos($table,"trends") !== false){
                    $fileds = "
                        MAX(h.value_max) as 'max',
                        AVG(h.value_avg) as 'avg',
                        MIN(h.value_min) as 'min'
                    ";
                }
                ...
                $sql = "
                    ...
                    WHERE h.clock >= {$stime}
                      AND h.`clock` <= {$etime}
                      AND h.`itemid` = ".zbx_dbstr($item['itemid'])."
                ";
                ...
                $byteStep = false;
                if($item['multiplier'] && $item['formula'] !== 1){
                    $result['max'] = $result['max'] * $item['formula'];
                    $result['min'] = $result['min'] * $item['formula'];
                    $result['avg'] = $result['avg'] * $item['formula'];
                }
                ...
                $result['max'] = convert_units(array(
                    'value' => $result['max'],
                    'units' => $item['units'],
                ...
                $result['min'] = convert_units(array(
                    'value' => $result['min'],
                    'units' => $item['units'],
                    'convert'        => ITEM_CONVERT_WITH_UNITS,
                    'byteStep'       => $byteStep,
                ));
                $result['avg'] =  convert_units(array(
                    'value' => $result['avg'],
                    'units' => $item['units'],
                    'convert'        => ITEM_CONVERT_WITH_UNITS,
                    'byteStep'       => $byteStep,
    ...
    function getCubismDataFromHistory(array $item,$stime = null,$period = SEC_PER_DAY){
        $result = array();
        $period1 = time() - 3600*24;
        if(!empty($stime) && !empty($period)){
            $stime = strtotime($stime);
            $etime = $stime + $period;
        }else{
        ...
        if($item){
            if($period1 > $etime && $item['value_type'] == 3){
                $table = "trends_uint";
            }elseif($period1 > $etime &&  $item['value_type'] == 0){
                $table = "trends";
    ...
    function getItem($option = array()){
        $hostid = !empty($option['hostid'])?$option['hostid']:null;
        $itemkey = !empty($option['itemkey'])?$option['itemkey']:null;
        $result = array();
        if(is_null($hostid) || is_null($itemkey)){
            return false;
        }
        $sql = "
            SELECT
            ...
            if($result['flags'] == 2){
                $sql = "
                    ...
                    FROM
                      item_discovery id,
                      items i
                    WHERE parent_itemid = {$result['itemid']}
                      AND i.`itemid` = id.`itemid`
                ";
                $discoveryItems = DBfetchArray(DBselect($sql));
    ...
    function getItemData(array $item = array(),$option = array()){
        $itemid = $item['itemid'];
        $table = $option['table'];
        $start = $option['start'];
        $end = $option['end'];
        ...
        if($table == 'trends'){
            $fields = "itemid,clock,value_avg AS 'value'";
        }else{

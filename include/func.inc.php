<?php
    ...
    **/
    ...
    function jsRedirect($url, $timeout = null) {
        $script = is_numeric($timeout) ? 'setTimeout(\'window.location="' . $url . '"\', ' . ($timeout * 1000) . ')' : 'window.location.replace("' . $url . '");';
        ...
        if (isset($msg)) {
            switch ($page['type']) {
                case PAGE_TYPE_IMAGE:
                    ...
                    $imageMessages[] = array(
                        'text'  => $msg,
                        'color' => (!$bool) ? array('R' => 255, 'G' => 0, 'B' => 0) : array(
                            'R' => 34,
                            'G' => 51,
                            'B' => 68
                        )
    ...
    function get_status() {
        ...
        // hosts
        $dbHosts = DBselect('SELECT COUNT(*) AS cnt,h.status' . ' FROM hosts h' . ' WHERE ' . dbConditionInt('h.status', array(
                HOST_STATUS_MONITORED,
                HOST_STATUS_NOT_MONITORED,
                HOST_STATUS_TEMPLATE
            )) . ' AND ' . dbConditionInt('h.flags', array(
    ...
    function uncheckTableRows($cookieId = null) {
        insert_js('cookie.eraseArray("cb_' . basename($_SERVER['SCRIPT_NAME'], '.php') . ($cookieId === null ? '' : '_' . $cookieId) . '")');
    }
